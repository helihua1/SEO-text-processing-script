...
#input_excel需要分析的文档 ：
input_excel = r"D:\sort\11.18测试文件\A.xlsx"
# source_path是需要不让其参与聚合的词：
source_path = r"D:\sort\11.18测试文件\数据清洗词.xlsx"  # 包含需要移除的词
# output_excel 是输出文档：
output_excel = fr'D:\sort\11.18测试文件\数据分析_3词_长度倍率{len_rate}X_词频倍率{num_rate_Input}X.xlsx'
...
def get_words_to_remove(source_path):
    ...
    if df.empty or df.shape[1] == 0:
        print(f"{source_path}文件为空或没有列，返回空列表")
        return []

    words = df.iloc[:, 0].dropna().astype(str).unique().tolist()

    if not words:
        print(f"{source_path}第一列没有有效词，返回空列表")
        return []
